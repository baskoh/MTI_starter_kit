#change the school and vectorstore text
import sqlite3
...
def link_users_to_app_function_ui(school_id):
    ...
    with sqlite3.connect(WORKING_DATABASE) as conn:
        ...
        selected_level_name = st.selectbox("Select Division (Optional, leave blank for all divisions):", [""] + list(level_choices.keys()))
        ...
        selected_class_name = st.selectbox("Select Branch (Optional, leave blank for all branches):", [""] + list(class_choices.keys()))
...
def update_prompt_template(profile_id, school_id_of_AD=None):
    """
    Display the UI to update prompt templates in Streamlit based on filter.

    :param profile_id: Profile ID of the logged-in user.
    :param school_id_of_AD: If the profile_id is AD, this specifies the school/statboard ID of the AD.
    """
...
def load_and_fetch_vectorstore_for_user(user_id):
    ...
    with sqlite3.connect(WORKING_DATABASE) as conn:
        ...
        if not vectorstore_data:
            st.warning("No knowledge base associated with the user.")
            return
        ...
        if not data_blob:
            st.warning("Vector stores data not found.")
            return
